...
*/
      ...
      const curVersion: number = await this.sqliteUtil.getVersion(this.database);
      this._isDbOpen = true;
  ...
  async getVersion(): Promise<number> {
    ...
    try {
      const currentVersion: number = await this.sqliteUtil.getVersion(this.database);
  ...
  async setSyncDate(syncDate: string): Promise<any> {
    ...
    try {
      ...
      const changes: number = await this.sqliteUtil.execute(this.database, stmt);
  ...
  async getSyncDate(): Promise<any> {
    ...
    try {
      ...
      const syncDate: number = await this.exportToJsonUtil.getSyncDate(this.database);
      if (syncDate > 0) {
        return { syncDate };
      } else {
