...
class ImportFromJson {
    ...
    class func createDatabaseSchema(dbHelper: DatabaseHelper, jsonSQLite: JsonSQLite, path: String,
        ...
        guard let mDB: OpaquePointer = try UtilsSQLite.getWritableDatabase(filename: path,
                    secret: secret) else {
        ...
        for ipos in 0..<jsonSQLite.tables.count {
            ...
            if let mSchema: [JsonColumn] = jsonSQLite.tables[ipos].schema {
                if mSchema.count > 0 {
                    let stmt: [String] = ImportFromJson.createTableSchema(mSchema: mSchema,
                                tableName: tableName, mode: mode)
                    statements.append(contentsOf: stmt)
                }
            }
            if let mIndexes: [JsonIndex] = jsonSQLite.tables[ipos].indexes {
                if mIndexes.count > 0 {
                    let stmt: [String] = ImportFromJson.createTableIndexes(mIndexes: mIndexes, tableName: tableName)
                    statements.append(contentsOf: stmt)
                }
            }
        }
    ...
    class func createTableSchema(mSchema: [JsonColumn], tableName: String, mode: String) -> [String] {
        var statements: [String] = []
        var stmt: String
        if mode == "full" {
    ...
    class func createTableIndexes(mIndexes: [JsonIndex], tableName: String) -> [String] {
        var statements: [String] = []
    ...
    class func createDatabaseData(dbHelper: DatabaseHelper, jsonSQLite: JsonSQLite, path: String,
                                  secret: String) throws -> Int {
        var changes: Int = -1
        ...
        do {
            ...
            throw DatabaseHelperError.createDatabaseData(message: message)
        }
        ...
        for ipos in 0..<jsonSQLite.tables.count {
            if let mValues = jsonSQLite.tables[ipos].values {
                if mValues.count > 0 {
                    isValue = true
                    do {
                        success = try ImportFromJson.createTableData(dbHelper: dbHelper, mDB: mDB,
                        mode: jsonSQLite.mode, mValues: mValues, tableName: jsonSQLite.tables[ipos].name)
        ...
        dbHelper.closeDB(mDB: mDB, method: "createTableData")
        return changes
    }

    // MARK: - ImportFromJson - createTableData

    class func createTableData(dbHelper: DatabaseHelper, mDB: OpaquePointer, mode: String,
                               mValues: [[UncertainValue<String, Int, Float>]], tableName: String) throws -> Bool {
        var success: Bool = false
        // Check if table exists
        do {
            let isTab: Bool = try UtilsJson.isTableExists(dbHelper: dbHelper, mDB: mDB,
                                tableName: tableName)
            if !isTab {
        ...
        do {
            jsonNamesTypes = try UtilsJson.getTableColumnNamesTypes(dbHelper: dbHelper, mDB: mDB,
            tableName: tableName)
        ...
        for jpos in 0..<mValues.count {
            // Check row validity
            let row: [UncertainValue<String, Int, Float>] = mValues[jpos]
            do {
                try UtilsJson.checkRowValidity(dbHelper: dbHelper, jsonNamesTypes: jsonNamesTypes,
            ...
            do {
                let data: [String: Any] = ["pos": jpos, "mode": mode, "tableName": tableName]
                let stmt: String = try createRowStatement(dbHelper: dbHelper, mDB: mDB, data: data,
                                                          row: row, jsonNamesTypes: jsonNamesTypes)
                let rowValues = UtilsJson.getValuesFromRow(rowValues: row)
                let lastId: Int = try dbHelper.prepareSQL(mDB: mDB, sql: stmt, values: rowValues)
                success = lastId != -1 ? true : false
            } catch DatabaseHelperError.createRowStatement(let message) {
                throw DatabaseHelperError.createTableData(message: message)
    ...
    class func createRowStatement(dbHelper: DatabaseHelper, mDB: OpaquePointer, data: [String: Any],
        ...
        if mode == "full" || (mode == "partial" && !retisIdExists) {
            ...
            let questionMarkString: String = UtilsJson.createQuestionMarkString(
                                                   length: jsonNamesTypes.names.count)
            stmt = "INSERT INTO \(tableName) (\(nameString)) VALUES (\(questionMarkString));"
        } else {
            // Update
            let setString: String = UtilsJson.setNameForUpdate(names: jsonNamesTypes.names)
            if setString.count == 0 {
                let message: String = "importFromJson: Table  \(tableName) values row " +
                    "\(pos) not set to String"
                throw DatabaseHelperError.createTableData(message: message)
            }
            if let rwValue: Any = row[0].value {
